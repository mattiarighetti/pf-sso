...
template::list::create \
	...
	q {
            hide_p 1
            values {$q $q}
            where_clause {UPPER(i.nome||i.cognome||e2.esame_id) LIKE UPPER('%$q%')}
        }
	rows_per_page {
	    label "Righe"
	...
	categoria_id {
	    hide_p 1
	    where_clause {((e2.categoria_id = :q_categoria_id AND :q_categoria_id <> 0) OR :q_categoria_id = 0)}
	}
    } \
    -orderby {
...
db_multirow \
	...
	reset_url
	delete_url 
    } esami query "SELECT e2.esame_id, '#'||e2.esame_id AS numero, i.cognome||' '||i.nome AS iscritto, c.descrizione
                       FROM pf_iscritti i, pf_categorie c, pf_esami e, pf_esami2 e2
                      WHERE e2.categoria_id = c.categoria_id
                        AND e.iscritto_id = i.iscritto_id
                        AND e.esame_id = e2.esame_id
                            [template::list::filter_where_clauses -name esami -and]
                   GROUP BY e2.esame_id, numero, iscritto, c.descrizione
