...
ad_form -name evento \
	...
	"SELECT evento_id, denominazione, descrizione, sala_id, start_time, end_time, prezzo::decimal FROM expo_eventi WHERE evento_id = :evento_id"
    } -new_data {
	...
	db_dml query "INSERT INTO expo_eventi (evento_id, denominazione, descrizione, sala_id, start_time, end_time, prezzo) VALUES (:evento_id, :denominazione, :descrizione, :sala_id, :dal, :al, :prezzo::money)"
    } -edit_data {
	set giorno [db_string query "select data from expo_edizioni where expo_id = :expo_id"]
        set dal [db_string query "select date :giorno + time :start_time"]
        set al [db_string query "select date :giorno + time :end_time"]
	db_dml query "UPDATE expo_eventi SET descrizione = :descrizione, sala_id = :sala_id, start_time = :dal, end_time = :al, prezzo = :prezzo::money WHERE evento_id = :evento_id"
    } -after_submit {
