...
ad_form -name evento \
	...
	if {$upload_file ne ""} {
	    ...
	    db_dml query "UPDATE eventi SET tipo_id = :tipo_id, data = :data, comune_id = :comune_id, docente_id = :docente_id  WHERE evento_id = :evento_id"
	}
    } -on_submit {
	if {$upload_file ne ""} {
	...
	ad_returnredirect -message "Evento (ID: $evento_id) creato correttamente." [export_vars -base "?template=eventi-list" {module}]
	ad_script_abort
    }
